...
export async function GET(req: Request) {
  ...
  const { data, error } = await supabase.from("push_subscriptions")
    .select("alert_all, lgas, stations, prefs_set_at")
  ...
  return NextResponse.json({
    prefs: { alertAll: data.alert_all, lgas: data.lgas, stations: data.stations },
...
export async function PUT(req: Request) {
  ...
  const { data, error } = await supabase.from("push_subscriptions").update({
    alert_all: prefs.alertAll, lgas: prefs.lgas, stations: prefs.stations, prefs_set_at: new Date().toISOString(),
